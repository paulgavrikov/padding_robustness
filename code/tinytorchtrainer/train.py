...
class Trainer:
    ...
    def prepare_model(args, in_channels, num_classes):
        ...
        if get_arg(args, "switch_conv_padding", None):
            for m in filter(lambda m: type(m) == torch.nn.Conv2d, model.modules()):
                m.padding_mode = args.switch_conv_padding
...
def main(args):
    ...
    dataset = data.get_dataset(args.dataset)(
        os.path.join(args.dataset_dir, args.dataset.split("@")[0])
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--switch_conv_padding", type=none2str, default=None)
