...
class CIFAR10Data():
    def __init__(self, root_dir, **kwargs):
        ...
        self.resolution = 32
        if kwargs.get("requested_resolution", -1) != -1:
            self.resolution = kwargs.get("requested_resolution")
        ...
        print("CIFAR10 WITHOUT RANDOM CROP!!!")
...
class OutpaintedCIFAR10Data():
    def __init__(self, root_dir, **kwargs):
        ...
        assert self.resolution <= 512, "Max. Resolution is 512x512 for OutpaintedCIFAR10 due to cropping"
    ...
    def train_dataloader(self, batch_size, num_workers, shuffle=True, drop_last=False, pin_memory=True, **kwargs):
        ...
        dataset = ImageFolder(root=path, transform=self.train_transform)
        dataloader = DataLoader(
            dataset,
            batch_size=batch_size,
            num_workers=num_workers,
...
class CINIC10Data():
    def __init__(self, root_dir, part="all", **kwargs):
        super().__init__()
...
class TensorData():
    def __init__(self, data_class, root_dir, **kwargs):
        super().__init__()
        self.root_dir = root_dir
...
class MNISTData(TensorData):
    ...
    def __init__(self, root_dir, **kwargs):
        super().__init__(MNIST, root_dir, **kwargs)
        self.mean = (0.1307,)
        self.std = (0.3081,)
        self.num_classes = 10
        self.in_channels = 1
...
class KMNISTData(TensorData):
    ...
    def __init__(self, root_dir, **kwargs):
        super().__init__(KMNIST, root_dir, **kwargs)
        self.mean = (0.1918,)
        self.std = (0.3483,)
        self.num_classes = 49
        self.in_channels = 1
...
class FashionMNISTData(TensorData):
    ...
    def __init__(self, root_dir, **kwargs):
        super().__init__(FashionMNIST, root_dir, **kwargs)
        self.mean = (0.2860,)
        self.std = (0.3530,)
        self.num_classes = 10
        self.in_channels = 1
...
class CIFAR10Corruptions(Dataset):
    ...
    def __init__(self, path: str, corruption: str, severity: int, transform=None, **kwargs):
        """
        Args:
...
all_datasets = {
    "cifar10": CIFAR10Data,
    "outpainted_cifar10": OutpaintedCIFAR10Data,
...
def get_dataset(name):
    ...
    if "@" in name:
        requested_resolution = int(name.split("@")[1])
        dataset_name = name.split("@")[0]

    return partial(all_datasets.get(dataset_name), requested_resolution=requested_resolution)
